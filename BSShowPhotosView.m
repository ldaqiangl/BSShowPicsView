...
- (void)updatePicCellWith:(NSIndexPath *)indexPath and:(NSString *)picName and:(NSString *)delImg andDelBlock:(void(^)(NSIndexPath *))delCom;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.photoDataSource.count > 0) {
        ...
        [cell updatePicCellWith:indexPath and:self.photoDataSource[indexPath.item] and:delImgName andDelBlock:^(NSIndexPath *cellIndexPath) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.item+1 == self.photoDataSource.count) {//添加图片
        ...
        if ([self.delegate respondsToSelector:@selector(showPhotosViewScanPics:withAllPicNames:)]) {
            
            [self.delegate showPhotosViewScanPics:self withAllPicNames:[self.photoDataSource subarrayWithRange:NSMakeRange(0, self.photoDataSource.count-1)]];
        }
